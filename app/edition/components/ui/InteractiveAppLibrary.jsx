...
const isEmptyFolderDropZoneId = (id) => String(id).startsWith(EMPTY_FOLDER_PREFIX);
const getFolderIdFromDropZoneId = (id) => id.replace(EMPTY_FOLDER_PREFIX, "");
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
const getIsDragIndented = (activatorEvent, deltaX, containerRef) => {
  const containerLeft = containerRef.current?.getBoundingClientRect().left ?? 0;
  const pe = activatorEvent;
  if (!pe?.clientX) return false;
  return pe.clientX + deltaX - containerLeft >= FOLDER_CONTENT_INDENTATION + 100;
};
const getIsDraggingUp = (dy) => dy < 0;
const getFolderId = (s) => {
  if (s.type === "folder") return s.id;
  return s.path ?? null;
};
const isAtFolderBoundary = (arr, oi, up) => {
  const o = arr[oi];
  if (!o) return false;
...
const getFolderIdAtBoundary = (arr, oi, up) => {
  const o = arr[oi];
  if (!o) return null;
...
const getDestinationFolderId = (arr, ai, oi, up, indent) => {
...
const moveFolderUtil = (arr, fi, di) => {
  const f = arr[fi];
  if (!f) return arr;
  let last = -1;
...
const snapCenterLeftToCursor = ({ activatorEvent, draggingNodeRect, transform }) => {
...
function useAppListDragState({ moduleSettings, setModuleSettings, containerRef }) {
  ...
  const handleDragMove = useCallback(({ delta, activatorEvent }) => {
  ...
  const handleDragEnd = useCallback(({ active, over, delta, activatorEvent }) => {
  ...
  return { activeItem, overTarget, dragDirection: isDraggingUp ? "up" : "down", destinationFolderId, handleDragStart, handleDragOver, handleDragMove, handleDragEnd };
}
...
function DropIndicator({ position, indented }) {
  return (
    <div style={{
...
function OverflowMenu({ isFolder, iuHidden, onRename, onDelete, onToggle }) {
  ...
  const Opt = ({ icon, text, destructive, onClick }) => (
...
/* ── Folder Name Input ── */
function FolderNameInput({ name, onNameChange, onDone }) {
  const ref = useRef(null);
...
/* ── Client Preview ── */
function ClientPreview({ items, inSplit = false }) {
...
/* ══════════════════════════════════════════════════════════════
   MAIN COMPONENT
   ══════════════════════════════════════════════════════════════ */
export function InteractiveAppLibrary({ inSplit = false }) {
  const [moduleSettings, setModuleSettings] = useState(DEFAULT_SETTINGS);

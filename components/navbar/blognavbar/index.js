...
export default function BlogNavbar({ isModule, headerIndex, isEnterPrice, tagData }) {
  ...
  //     const tagList = (await getAllTagWithSlug()) ?? [];
        ...
        <NavigationBlock>
          ...
          {tagData?.map((item, index) => {
            return (
              <SpanLink
                ...
                textColor={colorList?.fontColor}
                hoverColor={colorList?.primaryColor}
                className={router.pathname.startsWith('/features') ? 'active' : ''}>
          ...
          <SpanLink
            ...
            className={
              ...
              router.pathname.startsWith('/copilot-plus')
                ? 'active'
                : ''
            }>
  ...
  }, [colorList?.fontColor, colorList?.primaryColor, router.pathname, tagData]);
